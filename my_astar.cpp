...
int main()
{
	Mat img = imread("maze.jpg",CV_LOAD_IMAGE_COLOR);
	...
	float w_dijkstra = 0.1;
	float w_heuristic = 0.9;

	int mdfs[img.rows][img.cols];
	int mdfe[img.rows][img.cols];
	int md[img.rows][img.cols];
	...
	while(!q.empty())
	{
			...
			for( r=current.y-1 ; r<=current.y+1 ; r++ )
			{
				if(isValidPixel(r,c,img, is_visited))
				{
					...
					if(is_visited.at<uchar>(r,c) == 0)
					{
						...
						md[r][c] = (w_dijkstra)*mdfs[r][c] + (w_heuristic)*mdfe[r][c];
		...
		img.at<Vec3b>(to_be_pushed.y,to_be_pushed.x)[0] = 220;
...
/////////Code for backtracking////////////
	while(current!=start)
	{
		...
		current = temp;
		img.at<Vec3b>(current.y, current.x)[0] = 100;
		img.at<Vec3b>(current.y, current.x)[1] = 200;
		img.at<Vec3b>(current.y, current.x)[2] = 200;
		imshow("image", img);
		imshow("is_visited", is_visited);
